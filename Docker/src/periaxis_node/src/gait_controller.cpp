...
#include "periaxis_node/gait_controller.hpp"
...
class LegController {
	public:
		LegController(
			std::shared_ptr<GaitMaster> gaitMaster_node, //passed as shared_ptr, ref stored to class instance as weak_ptr in _gaitMaster_node
			...
			// _legPhaseOffset_ratio(legPhaseOffset_ratio),
			// _legPhaseOffset_count(legPhaseOffset_count)
		{
		...
		// Leg gait controller executing function
	...
	private:
		...
		//double _legPhaseOffset_ratio; //the ratio (% from 0-1) of the legs walk cycle time which will be offset (put out of phase)
		//double _legPhaseOffset_count; //the number of legPhaseOffset_ratio offsets to apply when calculating the legs CLK_Master cycle phasing
		...
		//Offset time to control phasing between each of the legs walk cycles; this is generated from: _legPhaseOffset_count * (_legPhaseOffset_ratio * cyclePeriod_millis)
		...
		// init tracked parameters related to leg timing and the transform to locate the traced function path above this leg
...
class GaitMaster : public rclcpp::Node { //, public std::enable_shared_from_this<GaitMaster>
	public:
		GaitMaster() : Node(PARENT_NODE_NAME) 
		{
			...
			_gaitParameters_sub = this->create_subscription<periaxis_interfaces::msg::GaitParameters>(
				...
				[this](const periaxis_interfaces::msg::GaitParameters::SharedPtr &msg) { // const pass-by-ref
			...
			for (int i = 0; i < _leg_count; ++i) {
				...
				_legControllers.emplace_back(std::make_unique<LegController>(gaitMaster_node_shared, _legGoalCoord_pub, 
			...
			RCLCPP_INFO(this->get_logger(), "gait_controller node initialised with %d legs and %dms publish rate.", _leg_count, _publishRate_ms);
		}
	...
	private:
		// Timer driven publishing loop to generate goal coords for each leg
		void updateGaits() {
		...
		// Callback for updating parameters based on messages received on periaxis_node/gait_params
		...
		// ## SET PRIVATE INTERNAL GAIT PARAMETERS ##
		// set internal master gait parameter _publishRate_ms
		bool set_publishRate_ms(const int newValue) {
			bool existingTimer = (_publish_timer); //evaluates false _publish_timer == nullptr
		...
		bool set_traceOffset_trans(const Eigen::Vector3d& new_translation) { // only set new value if within acceptable range
		...
		bool set_traceOffset_rot(const Eigen::Quaterniond& q) { // only set new value if within acceptable range
		...
		CLK::time_point _CLK_master = CLK::now(); //init to current time
		
		bool _reInitialiseFlag = false; //this controls when the leg class objects need to update their internal transforms and parameters after they have been updated from recieved periaxis_interfaces/gaitParameters message
		...
		const double _traceSTART = (3/10)*M_PI;
		const double _traceEND = (7/10)*M_PI;
		...
		const int _cyclePeriod_millis_MIN = 2000;
		const int _cyclePeriod_millis_MAX = 20000;
		const int _traceRadius_MIN = 1;
		const int _traceRadius_MAX = 15;
		const double _step_offsetFactor_MIN = 1;
		const double _step_offsetFactor_MAX = 50;
		const double _push_ratio_MIN = 0.1;
		const double _push_ratio_MAX = 0.9;
		// master trace translation limits
		const double _x_transRangeLimit_abs = 8;
		const double _y_transRangeLimit_abs = 8;
		const double _z_transRangeLimit_MIN = 5;
		const double _z_transRangeLimit_MAX = 20;
		
		// const double trans_xy_MIN = -8; //TBD MEASURE and better approach to bind leg valid area, trans bounds depends trace radius
		// const double trans_xy_MAX = 8; //TBD MEASURE and better approach to bind leg valid area, trans bounds depends trace radius
		// ## NEXT SECTION ##
};
...
int main(int argc, char** argv) {
    ...
    // Create and spin the ROS 2 node
    rclcpp::spin(std::make_shared<GaitMaster>());
