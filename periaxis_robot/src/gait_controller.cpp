...
const int publishPeriod_millis = 20; //Period for sending new coordinate topic messages
...
double traceRadius = 5; //Radius of the circle which is traced by the legs
...
Eigen::Vector3d traceOffset_trans = {0, 0, 20}; //translation offset for the frame which the trace function is drawn within        
...
class GaitMaster {
    ...
    public:
        GaitMaster(ros::NodeHandle *nh, int legNum, double legZAngle_rad, double legPhaseOffset_ratio, double legPhaseOffset_count) { //class constructor function
            ...
            //As the calculated phaseOffset_total can be more than a single cycle period, we take only the decimal part of the calculation; eg offset of -120% is equivelant to -20% in current cycle as it repeats
            ...
            // ie -40% offset between legs equates to the leg being 60% of the way through the previous cycle
        ...
        void walkingGait(CLK::time_point *CLK_master, bool me_reInitialise) {
