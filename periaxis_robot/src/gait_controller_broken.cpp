...
const double publishPeriod_millis = 50; //Period for sending new coordinate topic messages
const int cyclePeriod_millis = 10000; //Period for the complete PUSH & STEP cycle of a leg
const double push_ratio = 0.6; //ratio leg should be pushing vs stepping (eg (2/5) as 40% stepping, 60% pushing)

const double traceRadius = 10; //Radius of the circle which is traced by the legs
const double step_offsetFactor = 15; //effects the tip of the legs + Y and - Z axis offset from the trace path during a cycle resetting step, higher is larger step height
...
class GaitMaster {
    private:
        ...
        //ros::Subscriber gaitParameterUpdate_sub; //publisher for leg coord message type topic
        ...
        float traceOffset_trans_x;
        float traceOffset_trans_y;
        float traceOffset_trans_z;

        //These are the two matrixes which determine the final position and orientation of the trace path
        Eigen::Vector3d traceOffset_trans; //translation offset for the frame which the trace function is drawn within        
        ...
        //Transformation matrix for trace frame offset for leg and A_traceFrame template for final combined homogeneous transform matrix
        Eigen::Matrix3d legOffset_rot; //, traceOffset_rot;
    ...
    public:
        GaitMaster(ros::NodeHandle *nh, int legNum, double legZAngle_rad, double legPhaseOffset_ratio, double legPhaseOffset_count) { //class constructor function
            ...
            */
            ...
            //gaitParameterUpdate_sub = nh->subscribe<periaxis_robot::gaitParameters>("periaxis_robot/gaitParameters", 10, [=](auto &msg) {ParameterUpdate(*msg);});

            //initialise object leg number
            me_legNum = legNum;

            ROS_INFO_STREAM("IM OK 1 LEG " << me_legNum);
            ...
            legOffset_rot <<    cos(legZAngle_rad),  -sin(legZAngle_rad),   0,
                                sin(legZAngle_rad),  cos(legZAngle_rad),    0,
                                0,                   0,                     1;
            
            //initialise trace rotation to no rotation
            //traceOffset_rot = Eigen::Matrix3d::Identity();
            
            traceOffset_trans_x = 0;
            traceOffset_trans_y = 0;
            traceOffset_trans_z = 10;
            traceOffset_trans = {traceOffset_trans_x, traceOffset_trans_y, traceOffset_trans_z};

            //Build the complete trace frame transformation matrix from the leg rotation, trace frame rotation (post multiply due moving frame), and trace trame translation
            //Eigen::Matrix3d intra;
            //intra = legOffset_rot * traceOffset_rot;
            A_traceFrame.block<3,3>(0,0) << legOffset_rot; 
        ...
        //void ParameterUpdate(const periaxis_robot::gaitParameters& msg) {
            //ROS_INFO_STREAM("Parameter Update for leg " << me_legNum << " with new millis ");
        //}
