...
void setPositionCallback(const periaxis_robot::SetPosition::ConstPtr & msg)
{
  ...
  if(msg->arm1 == true) {
    dxl_comm_result = packetHandler->write2ByteTxOnly(portHandler, (uint8_t)msg->id1, ADDR_GOAL_POSITION, position1);//, &dxl_error);
    //if (dxl_comm_result == COMM_SUCCESS) {
  ...
  if(msg->arm2 == true) {
    dxl_comm_result = packetHandler->write2ByteTxOnly(portHandler, (uint8_t)msg->id2, ADDR_GOAL_POSITION, position2);//, &dxl_error);
    //if (dxl_comm_result == COMM_SUCCESS) {
  ...
  if(msg->arm3 == true) {
    dxl_comm_result = packetHandler->write2ByteTxOnly(portHandler, (uint8_t)msg->id3, ADDR_GOAL_POSITION, position3);//, &dxl_error);
    //if (dxl_comm_result == COMM_SUCCESS) {
...
/*
void setBulkPositionCallback(const periaxis_robot::SetPosition::ConstPtr & msg)
{
  //uint8_t dxl_error = 0;
  ...
  //Set servo 1 position
  if(msg->arm1 == true) {
    dxl_addparam_result = groupBulkWrite->addParam((uint8_t)msg->id1, ADDR_GOAL_POSITION, LEN_GOAL_POSITION, (uint8_t)&msg->position1);
    if (dxl_addparam_result == true) {
      ROS_INFO("setPosition : [ID:%d] [POSITION:%d]", msg->id1, msg->position1);
    } else {
      ROS_ERROR("Failed to set position! Result: %d", dxl_addparam_result);
    }
  ...
  //Set servo 2 position
  if(msg->arm2 == true) {
    dxl_addparam_result = groupBulkWrite->addParam((uint8_t)msg->id2, ADDR_GOAL_POSITION, LEN_GOAL_POSITION, (uint8_t)&msg->position2);
    if (dxl_addparam_result == true) {
      ROS_INFO("setPosition : [ID:%d] [POSITION:%d]", msg->id2, msg->position2);
    } else {
      ROS_ERROR("Failed to set position! Result: %d", dxl_addparam_result);
  ...
  //Set servo 3 position
  if(msg->arm3 == true) {
    dxl_addparam_result = groupBulkWrite->addParam((uint8_t)msg->id3, ADDR_GOAL_POSITION, LEN_GOAL_POSITION, (uint8_t)&msg->position3);
    if (dxl_addparam_result == true) {
      ROS_INFO("setPosition : [ID:%d] [POSITION:%d]", msg->id3, msg->position3);
    } else {
      ROS_ERROR("Failed to set position! Result: %d", dxl_addparam_result);
  ...
  //With all parameters set, send commands to servos
  dxl_comm_result = groupBulkWrite->txPacket();
  if(dxl_comm_result != COMM_SUCCESS) {
    ROS_ERROR("BulkWrite failed! Result: %d", packetHandler->getTxRxResult(dxl_comm_result));
  }
...
int main(int argc, char ** argv)
{
  ...
  //Initialise GroupBulkWrite instance
  ...
  ROS_INFO("Dynamixel read/write node running...");
  ...
  ros::Subscriber set_position_sub = nh.subscribe("dynamixel/set_position", 10, setPositionCallback);
  ros::spin();
