...
class GaitMaster {
    private:
        ...
        //TESTING BELOW 2
        Eigen::Matrix4d A_traceFrameTEST = Eigen::Matrix4d::Identity();
        Eigen::Matrix4d A_traceFrameTESTinverse = Eigen::Matrix4d::Identity();
    ...
    public:
        ...
        void GaitController(CLK::time_point *CLK_master, bool me_reInitialise) {
            ...
            //Calculate offset time for walk cycle phasing offset between legs based on current MASTER clock
            ...
            if(me_reInitialise) {
                //ROS_INFO_STREAM("1.elapsed" << elapsed.count() << " of " << cyclePeriod_millis);

                CLK_stepStart = *CLK_master; //reset the legs clock to master in preperation for re-initialisation calculation
                this->initialiseLegParameters();

                //After re-initialisation, CLK_stepStart is reset to begining of cycle, below ensures the new configuration continues from the current step progress
                CLK_stepStart -= std::chrono::milliseconds((int)( cycle_currentProgress * cyclePeriod_millis));
                //ROS_INFO_STREAM("2.elapsed " << std::chrono::duration_cast<std::chrono::milliseconds>(CLK_current - CLK_stepStart).count() << " offset calc " << ((1-cycle_currentProgress) * cyclePeriod_millis));
            }
            ...
            double TEMPcycle_currentProgress = cycle_currentProgress;
            ...
            if(publish_duration.count() > publishPeriod_millis) {
                ...
                //temp msg debugging
                ROS_INFO_STREAM("Leg current progress for leg " << me_legNum << " " << cycle_currentProgress << ". previously " << TEMPcycle_currentProgress);
                ...
                //Eigen::Vector3d v(traceCoord_trans(0), traceCoord_trans(1), traceCoord_trans(2));
                //traceCoord_trans = traceOffset_rot * v * traceOffset_rot.inverse();// * A_traceFrameTESTinverse; //q * p * q.inverse(); 
        ...
        void initialiseLegParameters() {
            ...
            legCycleOffset_millis = me_legPhaseOffset_count * (me_legPhaseOffset_ratio * cyclePeriod_millis);
            ...
            //Offset the current time based on the phase offset for the given leg
            //CLK_stepStart -= std::chrono::milliseconds((int)(legCycleOffset_millis));
            
            CLK::time_point CLK_currentTST = CLK_stepStart - std::chrono::milliseconds((int)legCycleOffset_millis);;

            ROS_INFO_STREAM("Leg " << me_legNum << " reinit legCycleOffset_millis " << legCycleOffset_millis << " " << std::chrono::duration_cast<std::chrono::milliseconds>(CLK_currentTST - CLK_stepStart).count());

            //Remove one cycle of time from step start so the correct cycle progress can be calcularted
            CLK_stepStart -= std::chrono::milliseconds((int)(cyclePeriod_millis));
            ...
            if(legCycleOffset_millis > cyclePeriod_millis) {
                ...
                ROS_INFO_STREAM("Removing " << cyclePeriod_millis * numCyclesAdjust << " numCyclesAdjust " << numCyclesAdjust);
            }
            ...
            A_traceFrame.block<3,3>(0,0) << (legOffset_rot); //combine both the trace frame rotation and the leg rotation
            A_traceFrame.block<3,1>(0,3) << traceOffset_trans;

            A_traceFrameTEST.block<3,3>(0,0) << (traceOffset_rot); //combine both the trace frame rotation and the leg rotation
            A_traceFrameTEST.block<3,1>(0,3) << 0, 0, 0;
