...
class LegManager {
    ...
    public:
        ...
        void invKinematicSolver(const periaxis_robot::legGoalCoord& msg) {
            ...
            const double base_radius = 5, L1 = 2.8, L2 = 9.4, L3 = 9.5;
            const short int ServoMIN = 180, ServoMAX = 840; //max and min numeric limits physically imposed by the attached servo C joint hardware
            const short int ServoMIN_J3 = 13, ServoMAX_J3 = 960; //different min max values for last fingure which has a wider range of motion
            ...
            bool invalid_theta1 = theta1_numeric < ServoMIN || theta1_numeric > ServoMAX;

            //Validate kinematic solution 1 is within servo bounds
            if(theta2_1_numeric < ServoMIN || theta2_1_numeric > ServoMAX || theta3_1_numeric < ServoMIN_J3 || theta3_1_numeric > ServoMAX_J3) {
                invalid_S1 = true;
            }
            //Validate kinematic solution 2 is within servo bounds
            if(theta2_2_numeric < ServoMIN || theta2_2_numeric > ServoMAX || theta3_2_numeric < ServoMIN_J3 || theta3_2_numeric > ServoMAX_J3) {
                invalid_S2 = true;
            }
            ...
            if( !(invalid_theta1 || invalid_S1 && invalid_S2) ) {
                ...
                /*
                //If the getPosition service is available, we can use this to find the kinematic solution which is closest to the servos current position 
                // (only if both kinematic solution options are available to select from)
                if(GetPosition_ServiceAvailable && !invalid_S1 && !invalid_S2) {
                    //Get position service type to retrieve current position of each servo joint
                    periaxis_robot::GetPosition srv;
                    short int j2_currentPosition, j3_currentPosition;
                    bool success;

                    //get current position of joint 2
                    srv.request.id = joint2.id;
                    success = ServoGetPosition_client.call(srv.request, srv.response);
                    if(success){
                        j2_currentPosition = srv.response.position;
                    }
                    //get current position of joint 3
                    srv.request.id = joint3.id;
                    success = ServoGetPosition_client.call(srv.request, srv.response);
                    if(success){
                        j3_currentPosition = srv.response.position;
                    }

                    //Find which solution is closest to current joint configuration
                    short int deltaJ2S1 = abs(j2_currentPosition - theta2_1_numeric); //Joint 2 S1 solution
                    short int deltaJ3S1 = abs(j3_currentPosition - theta3_1_numeric); //Joint 3 S1 solution
                    
                    short int deltaJ2S2 = abs(j2_currentPosition - theta2_2_numeric); //Joint 2 S2 solution
                    short int deltaJ3S2 = abs(j3_currentPosition - theta3_2_numeric); //Joint 3 S2 solution
                ...
                */
                ...
                if(!invalid_S1 && !invalid_S2 && lastCommand(legNum-1, 1) != -1 && lastCommand(legNum-1, 2) != -1 ) {
                    short int j2_previousPosition = lastCommand(legNum-1, 1); //matrix is 0 based, hence 1 is the second servo
